...
var initialHeaderTableSize = 4096
...
func TestWriteRST(t *testing.T) {
	...
	fr.WriteRSTStream(streamID, http2.ErrCode(errCode))
...
func TestWriteSettings(t *testing.T) {
	fr, buf := testFramer()
	settings := []http2.Setting{{ID: 1, Val: 2}, {ID: 3, Val: 4}}
...
func TestReadFrameOrder(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if !ok && err != http2.ConnectionError(http2.ErrCodeProtocol) {
...
func TestMetaFrameHeader(t *testing.T) {
	...
	tests := [...]struct {
		...
		4: {
			...
			want:              http2.ConnectionError(http2.ErrCodeCompression),
		},
		5: {
		...
		6: {
			...
			want:          streamError(1, http2.ErrCodeProtocol),
	...
	for i, tt := range tests {
		...
		if err != nil {
			...
			if se, ok := err.(http2.StreamError); ok && se.Cause != nil && se.Cause.Error() == tt.wantErrReason {
				se.Cause = nil
				got = se
			}
		}
...
func TestSettingsDuplicates(t *testing.T) {
	tests := []struct {
		...
		{nil, false},
		{[]http2.Setting{{ID: 1}}, false},
		{[]http2.Setting{{ID: 1}, {ID: 2}}, false},
		{[]http2.Setting{{ID: 1}, {ID: 2}}, false},
		{[]http2.Setting{{ID: 1}, {ID: 2}, {ID: 3}}, false},
		{[]http2.Setting{{ID: 1}, {ID: 2}, {ID: 3}}, false},
		{[]http2.Setting{{ID: 1}, {ID: 2}, {ID: 3}, {ID: 4}}, false},

		{[]http2.Setting{{ID: 1}, {ID: 2}, {ID: 3}, {ID: 2}}, true},
		{[]http2.Setting{{ID: 4}, {ID: 2}, {ID: 3}, {ID: 4}}, true},
