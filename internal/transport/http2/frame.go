...
// might be 0).
type frameParser func(fc *frameCache, fh FrameHeader, countError func(string), payload mem.BufferSlice, buf []byte) (Frame, error)
...
func NewFramer(w io.Writer, r *mem.BufferedReader) *Framer {
	fr := &Framer{
		w:          w,
		r:          r,
...
func (fr *Framer) ReadFrame() (Frame, error) {
	...
	bufSlice, err := fr.r.Read(int(fh.Length))
	if err != nil {
		return nil, err
	}
...
func parseDataFrame(fc *frameCache, fh FrameHeader, countError func(string), payload mem.BufferSlice, buf []byte) (Frame, error) {
	if fh.StreamID == 0 {
	...
	if fh.Flags.Has(FlagDataPadded) {
		...
		left, right := payload.SplitUnsafe(1)
		padSize = left.ReadOnlyData()[0]
		left.Free()
		payload = right
		if int(padSize)+1 > int(f.Length) {
	...
	if padSize > 0 {
		if int(padSize) == int(f.Length)-1 {
			payload.Free()
			payload = nil
		} else {
			left, right := payload.SplitUnsafe(int(f.Length) - 1 - int(padSize))
			right.Free()
			payload = left
		}
	}
...
func parseSettingsFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (Frame, error) {
	_ = bs.CopyTo(p)
	bs.Free()
...
func parsePingFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, payload []byte) (Frame, error) {
	_ = bs.CopyTo(payload)
	bs.Free()
	if len(payload) != 8 {
...
func parseGoAwayFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (Frame, error) {
	_ = bs.CopyTo(p)
	bs.Free()
	if fh.StreamID != 0 {
...
func parseUnknownFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, buf []byte) (Frame, error) {
	_ = bs.CopyTo(buf)
	bs.Free()
...
func parseWindowUpdateFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (Frame, error) {
	_ = bs.CopyTo(p)
	bs.Free()
	if len(p) != 4 {
	...
	if inc == 0 {
		...
		return nil, streamError(fh.StreamID, http2.ErrCodeProtocol)
	}
...
func parseHeadersFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (_ Frame, err error) {
	_ = bs.CopyTo(p)
	bs.Free()
	hf := &HeadersFrame{
		FrameHeader: fh,
...
func parsePriorityFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, payload []byte) (Frame, error) {
	_ = bs.CopyTo(payload)
	bs.Free()
	if fh.StreamID == 0 {
...
func parseRSTStreamFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (Frame, error) {
	_ = bs.CopyTo(p)
	bs.Free()
	if len(p) != 4 {
...
func parseContinuationFrame(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (Frame, error) {
	_ = bs.CopyTo(p)
	bs.Free()
	if fh.StreamID == 0 {
...
func parsePushPromise(_ *frameCache, fh FrameHeader, countError func(string), bs mem.BufferSlice, p []byte) (_ Frame, err error) {
	_ = bs.CopyTo(p)
	bs.Free()
	pp := &PushPromiseFrame{
		FrameHeader: fh,
...
func summarizeFrame(f Frame) string {
	...
	case *DataFrame:
		...
		if len(data) > max {
			fmt.Fprintf(&buf, " (%d bytes omitted)", len(data)-max)
		}
